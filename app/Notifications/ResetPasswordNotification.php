...
class ResetPasswordNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        return (new MailMessage)->greeting('Hola '.$notifiable->name)
            ->subject(Lang::getFromJson('Solicitud de reestablecimiento de contraseña '))
            ->line(Lang::getFromJson('Recibes este email porque se solicito un reestablecimiento de contraseña para tu cuenta. '))
            ->action(Lang::getFromJson('Reestablecer contraseña'), url(config('app.url').":8000".route('password.reset', ['token' => $this->token, 'email' => $notifiable->getEmailForPasswordReset()], false)))
